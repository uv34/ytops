...
class FollowFrame(BaseFrame):
    ...
    def create_widgets(self):
        ...
        tk.Button(btn_frame, text="Cancel", bg="#3e3e3e", fg="white", width=10, command=self.destroy).pack(side="left",
                                                                                                           padx=5)
    ...
    def update_suggestions(self, event):
        ...
        for suggestion in suggestions:
            row = tk.Frame(self.suggestion_frame, bg="#2e2e2e");
...
class PlaylistInfoFrame(BaseFrame):
    ...
    def _add_song_row(self, parent, song):
        ...
        tk.Label(row, text=f"{song.name} - {song.author}"[:12] + "...", bg="#2e2e2e", fg="white", anchor="w").pack(
    ...
    def upload_image(self):
        def task():
            ...
            if path:
                ...
                tk_img = ImageTk.PhotoImage(img)
                self.image_btn.config(image=tk_img);
                self.image_btn.image = tk_img
                self.filepath = path
